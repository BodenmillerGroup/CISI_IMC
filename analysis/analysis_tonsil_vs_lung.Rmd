...
```{r plot_scatterplots, fig.height = 12, fig.width = 10, fig.align = "center", results="asis"}
...
all.clusters <- lapply(keep(X.list, function(x){
  metadata(x)$dataset=="lung"}), 
  function(sce){
  mat <- t(assay(sce, "exprs"))
  out1 <- Rphenograph(mat, k=80)
...
for (i in unique(all.clusters$k)){
  ...
  for (d in unique(all.clusters$dataset)){
    ...
    col_anno <- columnAnnotation(celltypes=anno_barplot(table(all.clusters %>% 
                                                                dplyr::filter(k==i & dataset==d) %>% 
                                                                pull(ground_truth), 
                                                              all.clusters %>% 
                                                                dplyr::filter(k==i & dataset==d) %>% 
                                                        ...
                                                        bar_width = 1, 
                                                        height = unit(5, "cm")))
    row_anno <- rowAnnotation(celltypes=anno_barplot(table(all.clusters %>% 
                                                             dplyr::filter(k==i & dataset==d) %>% 
                                                             pull(simulated), 
                                                           all.clusters %>% 
                                                             dplyr::filter(k==i & dataset==d) %>% 
    ...
    ht_list <- append(ht_list, 
                      list(grid.grabExpr(draw(Heatmap(temp.matrix,
                                              ...
                                              heatmap_legend_list=list(Legend(labels=names(celltypes_col), 
                                                                              title="Celltypes", 
