...
```{r plot_normalizations, fig.height = 8, fig.width = 18, fig.align = "center"}
...
```{r test, fig.height = 7, fig.width = 10, fig.align = "center", fig.keep="last"}
# # Add all X_test counts together into dataframe for easier plotting
# # Calculate correlations between ground truth and simulated data for each protein
# X.corCell <- X.df %>%
#   group_by(k, dataset, cell) %>%
#   summarize(correlation=cor(ground_truth, simulated),
#             n_zeros=sum(ground_truth==0))
# 
# # Plot correlations for k=1 for each test/training dataset combination
# for (i in c("n_zeros")) {
#   cat('#####', i, '\n')
#   
#   proteinChar <- ggscatter(X.corCell %>%
#                              dplyr::filter(k=="1"), 
#                            x="correlation", y=i,
#                            add="reg.line",
#                            color=pal_npg("nrc")("1"),
...
Correlation between ground truth and decomposed results per protein for k=1 and 
asinh transformed counts compared to protein characteristics of gorund truth:
...
#### Cell Phenotyping (k=1, Immucan_lung as test set, arcsinh)

Next, we use a pre-trained random forest classifier to predict celltypes of decomposed
Immucan lung test datasets using k=1 and the arcsinh transformed decomposed counts.
