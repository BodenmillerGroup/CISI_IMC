...
plot.folder <- file.path(thesis_output.folder, "analysis_tonsil_vs_lung")
...
dataset (created by tonsil_vs_lung.py). (Note: Since the training was done on normalized 
counts using the original normalization used by CISI, e.g. protein expressions 
are normalized accross cells, all counts shown in this script are actually normalized. 
If arcsinh is applied, then this is applied on top of the before mentioned normalization.)
...
In the next part, we have a look at the result statistics and outputs produced 
by the CISI training function.
...
Next, we have a look at the dictionaries U created for the tonsil and lung datasets.
For this, the active proteins in a module are defined as the proteins explaining 90% 
of the cumulative sum of the whole module. These are then coloured accordingly.

#### Heatmaps of U {.tabset}

```{r plot_U, results="asis", fig.height=8, fig.width=22, fig.align="center"}
...
To illustrate the overlap of modules between datasets, both U's are shown in the 
same heatmap and shared modules are coloured accordingly.
...
To have a better intuition on how CISI performs, we now look at the expression levels 
of individual cells for the testing image.

#### Test Image *(Tonsil th152, k=1)*  {.tabset}

```{r plot_X, fig.height=10, fig.width=10, fig.align="center", fig.keep=c(13, 26), results="asis"}
...
```{r save_plots_1}
# Correct for outlier in SMA
X.tonsil1 <- lapply(X.tonsil1, function(sce.temp){
  # Find outlier and change to second highest value for SMA
  counts(sce.temp)["SMA", 
                   which.max(counts(sce.temp)["SMA", ])] <- sort(counts(sce.temp)["SMA", ],
                                                                 TRUE)[2]
  sce.temp
})

# Create s_img
s_img_1.plot <- plot_grid(plotlist=plot_cells(X.tonsil1, 
                                              masks.tonsil["20220520_TsH_th152_cisi1_002"], "MPO",
                                              ft_size=thesis_title.fontsize),
                          nrow=1, labels=c("", names(X.tonsil1)[1], "", names(X.tonsil1)[2]),
                          rel_widths=c(1,2,1,2),
                          label_size=thesis_title.fontsize, hjust=-1)
s_img_2.plot <- plot_grid(plotlist=plot_cells(X.tonsil1, 
                                              masks.tonsil["20220520_TsH_th152_cisi1_002"], "SMA", 
                                              ft_size=thesis_title.fontsize),
                          nrow=1, labels=c("", names(X.tonsil1)[1], "", names(X.tonsil1)[2]),
                          rel_widths=c(1,2,1,2),
                          label_size=thesis_title.fontsize, hjust=-1)

s_img.grid <- plot_grid(s_img_1.plot, s_img_2.plot,
                        ncol=1, labels="AUTO",
                        label_size=thesis_title.fontsize)
# Save s_img
save_plot(file.path(plot.folder, "s_img.pdf"),
          s_img.grid, base_width=6.5, base_height=6)
```

#### CD20 of Test Image Overlayed (Tonsil th152, k=1)

To make it easier to see differences in simulated and ground truth images, we overlay 
them next using opposite colors. Cells which have similar expressions in both simulated 
and ground truth data should therefore have an orange colour.
...
In the next part, we look at the counts of individual cells for different protein 
combinations.

#### Scatterplots of Counts *(Immucan Lung)* {.tabset}
...
To have an idea on how individual proteins perform using CISI and not just an overview, 
we also look at individual protein performances.
...
```{r results_per_protein, fig.height=10, fig.width=10, fig.align="center", results="asis"}
...
# Create s_protein plot 1 and 2
s_protein_1.plot <- plot_protein_scatterplot(X.df %>%
                                               dplyr::filter(k=="1" & dataset=="lung_lung"),
                                             ncol=4, cor_size=2.5) +
  theme_cowplot(thesis_title.fontsize) 
  # theme(legend.position="bottom", legend.box="vertical",
  #       axis.text.x=element_text(angle=45, vjust=1, hjust=1)) +
  # scale_x_discrete(guide=guide_axis(n.dodge=1))
s_protein_2.plot <- plot_protein_scatterplot(X.df %>%
                                               dplyr::filter(k=="1" & dataset=="tonsil_tonsil"),
                                             ncol=4, cor_size=2.5) +
  theme_cowplot(thesis_title.fontsize) 
  # theme(legend.position="bottom", legend.box="vertical",
  #       axis.text.x=element_text(angle=45, vjust=1, hjust=1)) +
...
# Create s_protein plot 3 and 4
s_protein_3.plot <-  plot_protein_cor(X.cor %>%
                                        dplyr::filter(dataset=="lung_lung")) +
  theme_cowplot(thesis_title.fontsize)
s_protein_4.plot <-  plot_protein_cor(X.cor %>%
                                        dplyr::filter(dataset=="tonsil_tonsil")) +
  theme_cowplot(thesis_title.fontsize)
```
...
```{r results_per_protein_dist, fig.height=7, fig.width=12, fig.align="center"}
...
# Create s_protein_snr plot
s_protein_snr.plot <- ggplot(X.cor, 
                        aes(x=snr, y=correlation, label=protein)) +
    geom_point(color=pal_npg("nrc")("1")) +
    facet_wrap(~ dataset, scales="free", ncol=2) +
    stat_cor(size=2.5,
             label.x.npc="center",
...
shown using k=1. The cells are coloured by their celltype (predicted by Daniel and 
present in the training data already).

```{r clustering, fig.height=5, fig.width=10, fig.align="center", results="asis"}
# Plot UMAP and TSNE for k=1 and lung data
# Create empty list for plots to save
s_low_dim.list <- list()
s_low_dim.names <- list()
for (sce in keep(X.lung_lung, function(x){metadata(x)$k=="1"})){
  ...
  for (method in c("UMAP", "TSNE")){
    X.redDim_temp <- dittoDimPlot(sce,
                                  var="celltype",
    ...
    X.redDimlist <- append(X.redDimlist,
                           list(X.redDim_temp))
    s_low_dim.list <- append(s_low_dim.list,
                             list(X.redDim_temp +
                                    theme_cowplot(thesis_title.fontsize)))
    s_low_dim.names <- append(s_low_dim.names, paste(metadata(sce)$ground_truth, 
                                                     method, " "))
  }
...
# Initialize empty list for saving heatmaps for thesis
s_clustering.list <- list()
...
for (i in unique(all.clusters$k)){
  ...
  for (d in unique(all.clusters$dataset)){
    ...
    if(i=="1" & d=="lung_lung"){
      ...
      anno.list <- get_anno_clusters(all.clusters %>% 
                                       dplyr::filter(k==i & dataset==d),
                                     celltype.col=celltype.col, size=1.5,
                                     ft_size=thesis_axis_title.fontsize)
      col_anno <- anno.list[[1]]
      row_anno <- anno.list[[2]]
      
      s_clustering.list <- append(s_clustering.list,
                                  list(grid.grabExpr(draw(Heatmap(temp.matrix,
                                                                  ...
                                                                  column_title=d,
                                                                  row_names_gp=gpar(
                                                                    fontsize=thesis_axis_title.fontsize),
                                                                  column_names_gp=gpar(
                                                                    fontsize=thesis_axis_title.fontsize),
                                                                  column_title_gp=gpar(
                                                                    fontsize=thesis_title.fontsize,
                                                                    fontface="bold"),
                                                                  rect_gp=gpar(col="white", lwd=1),
...
We also compare the clusters of the ground truth data to the clusters of the simulated
...
for (i in unique(all.clusters$k)){
  ...
  for (d in unique(all.clusters$dataset)){
    ...
    if(i=="1" & d=="lung_lung"){
      ...
      s_clustering.list <- append(s_clustering.list,
                                  list(grid.grabExpr(draw(Heatmap(t(clusters.kl),
                                                                  show_row_dend=FALSE,
                                                                  show_column_dend=FALSE,
                                                                  top_annotation=col_anno,
                                                                  left_annotation=row_anno,
                                                                  col=colorRamp2(c(0, 0.1, 5),
                                                                                 rev(magma(3))),
                                                                  column_title=d,
                                                                  row_names_gp=gpar(fontsize=thesis_axis_title.fontsize),
                                                                  column_names_gp=gpar(fontsize=thesis_axis_title.fontsize),
                                                                  column_title_gp=gpar(fontsize=thesis_title.fontsize, fontface="bold"), 
                                                                  rect_gp=gpar(col="white", lwd=1),
                                                                  heatmap_legend_param=list(
                                                                    title="Kullback-Leibner Divergence",
                                                                                            labels_gp=gpar(fontsize=thesis_axis_title.fontsize),
                                                                                            title_gp=gpar(fontsize=thesis_axis_title.fontsize, fontface="bold"))), 
                                                          heatmap_legend_list=list(
                                                            Legend(labels=names(celltype.col), 
                                                                   title="Celltypes",
                                                                   legend_gp=gpar(fill=celltype.col),
                                                                   labels_gp=gpar(
                                                                     fontsize=thesis_axis_title.fontsize),
                                                                   title_gp=gpar(
                                                                     fontsize=thesis_axis_title.fontsize, fontface="bold")))))))
    }
  }
...
in each cluster and then compare it to the celltypes of each cluster. Ideally,
each cluster has a median protein expression pattern that fits its main celltype.

```{r clustering_exprs, fig.height=14, fig.width=10, fig.align="center", results="asis"}
for (i in unique(all.clusters$k)){
  cat("##### k =", i, "\n")
  ...
  for (d in unique(all.clusters$dataset)){
    ...
    cluster_gt.exprs <- assay(X.lung[grepl(i, names(X.lung)) &
                                       ...
                                       grepl("ground_truth", names(X.lung))][[1]], "counts") %>%
      as.data.frame() %>%
      mutate(protein=rownames(.)) %>%
      melt() %>%
      dplyr::rename(cell=variable, expr=value) %>%
      group_by(protein) %>%
      mutate(expr=(expr-min(expr))/max(expr)) %>%
      ungroup()
    ...
    if(i=="1" & d=="lung_lung"){
      ...
      anno.list <- get_anno_clusters(all.clusters %>% 
                                       dplyr::filter(k==i & dataset==d),
                                     celltype.col=celltype.col, direction="normal", 
                                     which_sim="column", size=1.5,
                                     ft_size=thesis_axis_title.fontsize)
      col_anno <- anno.list[[1]]
      row_anno <- anno.list[[2]]
      s_clustering.exprGT <- Heatmap(cluster_gt.exprs,
                                     show_row_dend=FALSE,
                                     show_column_dend=FALSE,
                                     top_annotation=col_anno,
                                     col=magma(100),
                                     ...
                                     column_title_gp=gpar(fontsize=thesis_title.fontsize,
                                                          fontface="bold"),
                                     rect_gp=gpar(col="white", lwd=1),
                                     heatmap_legend_param=list(title="ground_truth",
                                                               labels_gp=gpar(fontsize=thesis_axis_title.fontsize), title_gp=gpar(fontsize=thesis_axis_title.fontsize, fontface="bold")))
      s_clustering.exprSIM <- Heatmap(cluster_sim.exprs,
                                      show_row_dend=FALSE,
                                      show_column_dend=FALSE,
                                      top_annotation=row_anno,
                                      col=magma(100),
      ...
      s_clustering.list <- append(s_clustering.list,
                                  list(grid.grabExpr(
                                    draw(s_clustering.exprGT + s_clustering.exprSIM,
                                         heatmap_legend_list=list(
                                           Legend(labels=names(celltype.col),
                                                  title="Celltypes",
                                                  legend_gp=gpar(
...
```{r save_plots}
# Save d_comb_res
thesis_res.plot <- plot_cisi_results(res %>%
                                       dplyr::select(dataset, training, 
                                                     simulation, k, `Gene average`) %>%
                                       mutate(group=paste(training, dataset, sep="_")), 
                                     "group", "Gene average", "k")

ggsave(file.path(plot.folder, "d_comb_res.pdf"), 
       thesis_res.plot +
         theme_cowplot(thesis_title.fontsize) +
         labs(x="Training_Decomposition Datasets",
              y=clean_plot_labels(thesis_res.plot$labels$y),
              pattern=clean_plot_labels(thesis_res.plot$labels$pattern)) +
         theme(legend.position="bottom", legend.box="vertical", 
               axis.text.x=element_text(angle=45, vjust=1, hjust=1)) +
         scale_x_discrete(guide=guide_axis(n.dodge=1)), 
...
# Save r_dictionary
plot_U_thesis(u, "k", "dataset", "1", file.path(plot.folder, "r_dictionary.pdf"))
# Save r_dictionary_comb
plot_U_membership_thesis(u, "k", "dataset", "1", file.path(plot.folder, "r_dictionary_comb.pdf"))

# Save s_protein
s_protein_1.grid <- plot_grid(s_protein_1.plot, ncol=1, labels="A", 
                              align="h", axis="l")
s_protein_2.grid <- plot_grid(s_protein_2.plot, ncol=1, labels="B", 
                              align="h", axis="l")
s_protein_3.grid <- plot_grid(s_protein_3.plot +
                                labs(x=clean_plot_labels(s_protein_3.plot$labels$x),
                                     y=clean_plot_labels(s_protein_3.plot$labels$y)), 
                              s_protein_4.plot +
                                labs(x=clean_plot_labels(s_protein_4.plot$labels$x),
                                     y=clean_plot_labels(s_protein_4.plot$labels$y)),
                              ncol=1, labels="AUTO", 
                              align="h", axis="l")

# Save s_protein_scatter
save_plot(file.path(plot.folder, "s_protein_scatter_L.pdf"),
          s_protein_1.grid, base_width=6, base_height=9)
save_plot(file.path(plot.folder, "s_protein_scatter_T.pdf"),
          s_protein_2.grid, base_width=6, base_height=9)
save_plot(file.path(plot.folder, "s_protein_cor.pdf"),
          s_protein_3.grid, base_width=4.33, base_height=9)

# Save s_protein_snr 
ggsave(file.path(plot.folder, "s_protein_snr.pdf"), 
       s_protein_snr.plot +
         labs(x=toupper(s_protein_snr.plot$labels$x),
              y=clean_plot_labels(s_protein_snr.plot$labels$y)), 
       width=15, height=9, units="cm")

# Create s_low_dim
s_low_dim.list <- lapply(s_low_dim.list, function(p){
  p + theme(legend.position="bottom", legend.box="vertical") +
    guides(color=guide_legend(ncol=3, override.aes=list(size=5)))
})
s_low_dim.grid <- plot_grid(plot_grid(plotlist=s_low_dim.list[1:2], ncol=2,
                                      labels=s_low_dim.names[1:2], 
                                      align="v", axis="b", label_size=thesis_title.fontsize), 
                            plot_grid(plotlist=s_low_dim.list[3:4], 
                                      labels=s_low_dim.names[3:4], align="v", 
                                      axis="b", label_size=thesis_title.fontsize),
                            ncol=1, labels="AUTO", align="h", axis="l")
# Save s_low_dim
save_plot(file.path(plot.folder, "s_low_dim.pdf"),
          s_low_dim.grid, base_width=6.4, base_height=9)

# Create s_clustering plots
s_clustering1.grid <- plot_grid(plotlist=s_clustering.list[1:2], 
                                ncol=1, labels="AUTO", align="h", axis="l")
s_clustering2.grid <- plot_grid(plotlist=s_clustering.list[3], 
                                ncol=1, labels="C", align="h", axis="l")

# Save s_clustering
save_plot(file.path(plot.folder, "s_clustering_1.pdf"),
          s_clustering1.grid, base_width=6.5, base_height=9)
save_plot(file.path(plot.folder, "s_clustering_2.pdf"),
          s_clustering2.grid, base_width=6.5, base_height=6)
```