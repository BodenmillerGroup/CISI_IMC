...
### Transformation of X (before SMAF training)
...
```{r test_U_clustered}
# # TODO: remove
# plot_U_clustered <- function(df, iter_var, repetition){
#   for (i in unique(df[[iter_var]])) {
#     cat('#####', iter_var, ' = ', i, '\n')
#     
#     # Create empty HeatmapList and add all repetitions as individual heatmaps
...
#         as.matrix()
#       
#       colnames(res.temp) <- paste(r, colnames(res.temp), sep="_")
#       combined.matrix <- cbind(combined.matrix, res.temp)
#     }
#     
#     col.anno <-columnAnnotation(dataset=gsub("_.*", "", colnames(combined.matrix)))
#     Heatmap(combined.matrix, column_title = "",
#             col=col_fun, show_heatmap_legend=FALSE, 
#             show_row_dend=FALSE, row_names_gp=gpar(fontsize = 8),
#             column_names_gp=gpar(fontsize=8),
#             column_title_gp=gpar(fontsize=10),
...
#### Mantel test for different datasets
...
```{r plot_tonsil_vs_lung_X, fig.height = 20, fig.width = 10, fig.align = "center", fig.keep="last"}
tonsil_vs_lung_X.plotList <- keep(tonsil_vs_lung_X.list, function(x){
  metadata(x)$training=="tonsil" & metadata(x)$dataset=="tonsil" & metadata(x)$k=="3"})
...
tonsil_vs_lung_X.imgList <- plot_cells(tonsil_vs_lung_X.plotList, masks.tonsil, 
                                       "GranzymeB")
tonsil_vs_lung_X.img <- plot_grid(plotlist=append(tonsil_vs_lung_X.imgList[-(1:2)],
                                                        tonsil_vs_lung_X.imgList[1:2]),
                                          ncol=2,labels=tonsil_vs_lung_X.plotListNames,
                                          label_size=15, hjust=c(-2, -1.5), 
                                          vjust=1, scale=0.9)
tonsil_vs_lung_X.img
```

```{r plot_tonsil_vs_lung_X_diff, fig.height = 20, fig.width = 10, fig.align = "center", fig.keep="last"}
tonsil_vs_lung_X.imgListExprs <- plot_cells(tonsil_vs_lung_X.plotList, masks.tonsil,
                                            "GranzymeB", layer="exprs")
tonsil_vs_lung_X.imgExprs <- plot_grid(plotlist=append(tonsil_vs_lung_X.imgListExprs[-(1:2)],
                                                       tonsil_vs_lung_X.imgListExprs[1:2]),
                                       ncol=2, labels=tonsil_vs_lung_X.plotListNames,
                                       label_size=15, hjust=c(-2, -1.5),
                                       vjust=1, scale=0.9)
tonsil_vs_lung_X.imgExprs
```
