...
possible_patterns <- c("none", "circle", "crosshatch", "pch", "stripe")
```
...
plot_results <- function(df, cor, group, fill, pattern){
  ...
  barplot <- ggplot(df, aes(x=!!sym(group), y=!!sym(cor),
    ...
    scale_pattern_manual(values=possible_patterns[1:length(unique(df[[pattern]]))], 
                         labels=unique(df[[pattern]])) +
    labs(x=group, y=measure, pattern=pattern) + 
...
plot_results_grid <- function(df, param){
  ...
  lineplot <- ggplot(df, aes(!!sym(param), `Gene average`)) +
    facet_grid(parameters[parameters!=param][1] ~ parameters[parameters!=param][2]) +
    theme_cowplot(title.fontsize) +
...
plot_U <- function(u.temp, title, thresh=0.9){
  ...
  u.membership <- apply(u.temp, 2, functio(u.col){
...
```{r plot_average_protein_correlation, results="asis", fig.asp=0.4, fig.width=10, fig.align="center"}
...
```{r results_per_protein_cor, fig.asp=1, fig.width=10, fig.align="center"}
aoi <- "exprs"
...
```{r plot_protein_expr, fig.asp=0.3, fig.width=10, fig.align="center", fig.keep=c(13, 26), results="asis"}
...
```{r plot_U, fig.asp=0.7, fig.width=10, fig.align="center"}
# Plot U for best run
...
```{r plot_A, fig.asp=0.6, fig.width=10, fig.align="center"}
# Plot A for best run
a.plot <- plot_A(a, "A")
print(a.plot)
```