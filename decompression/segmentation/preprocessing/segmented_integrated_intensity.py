...
if __name__ == '__main__':
	...
	parser.add_argument('--tissues', help='Comma-separated list of tissue numbers to include',default='4')
	parser.add_argument('--stitched-subdir', help='Subdirectory with stitched images', default='original')
	parser.add_argument('--mask-basepath', help='Optional alternative basepath for cell masks', default=None)
	parser.add_argument('--pixel-size', help='Pixel size in microns',type=float,default=1)
	parser.add_argument('--min-cell-area', help='Minimum area threshold for cells (in microns^2)',type=float,default=15)
	...
	parser.set_defaults(as_average=True)
	...
	for t in args.tissues.split(','):
		...
		CellMasks_fT = load_npz('%s/%s/segmented/cell_masks.npz' % (args.mask_basepath,tissue))
		if args.as_average:
		...
		FP_mask = glob.glob(os.path.join(args.mask_basepath,tissue,'segmented','mask.tiff'))
		if len(FP_mask) == 0:
		...
		if (tissue == 'tissueX') and ('Foxp2' in L): # Foxp2 was measured twice in tissue2
			gi = L.index('Foxp2')
			X_direct.append(X_direct[gi])
			L.append(L[gi])
		if (tissue == 'tissueX'): # Pdgfra and Prox1 validation did not appear to work (Pdgfra looks good in tissue12)
			gi = L.index('Pdgfra')
		...
		include = find_inliers(X,X_direct,CellMasks_fT,args.pixel_size,args.min_cell_area,args.max_cell_area)
