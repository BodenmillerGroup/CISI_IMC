...
if __name__ == '__main__':
	...
	parser.add_argument('--basepath', help='Path to directory with subdirs for parsed images in each tissue',default="preprocessing")
	parser.add_argument('--tissues', help='Comma-separated list of tissue numbers to include',default="4,5")
	...
	f = open('%s/data/selected_genes.txt' % args.trainpath)
	AllGenes = np.array([line.strip() for line in f])
	f.close()
	phi = np.load('%s/result/10_measurements/2_max/phi.npy' % args.trainpath)
	phi_corr = (np.einsum('ij,ik->jk',phi,phi)/phi.sum(0)).T - np.eye(phi.shape[1])
	relative_abundance = np.load('%s/data/relative_abundance.npy' % args.trainpath)
	expression_level = np.load('%s/data/average_on_expression_level.npy' % args.trainpath)
#	train_corr = np.load('%s/correlations.npy' % args.trainpath)
#	train_corr = distance.squareform(train_corr)
	U = np.load('%s/result/10_measurements/2_max/%s.npy' % (args.trainpath, args.modules))
	...
	for ti,t in enumerate(T):
		tissue = 'tissue%d' % t
		composite_measurements = np.load('%s/output/%s/%s/composite_measurements.npy' % (args.basepath, tissue,args.method))
		direct_measurements = np.load('%s/output/%s/%s/direct_measurements.npy' % (args.basepath, tissue,args.method))
		direct_labels = np.load('%s/output/%s/%s/direct_labels.npy' % (args.basepath, tissue, args.method))
		if args.region_mask is not None:
			CellMasks = load_npz('%s/output/%s/segmented/cell_masks.size_threshold.npz' % (args.basepath,tissue))
			region_mask = imageio.imread('%s/output/%s/stitched_aligned_filtered/%s' % (args.basepath,tissue,args.region_mask))
