...
# 	parser.add_argument('--max-composition', help="Maximum times each gene is represented (mode G), or max genes per composition (mode M)", type=int)
# 	parser.add_argument('--mode', help="Mode in which to apply max-compositions", default="G")
# 	args,_ = parser.parse_known_args()
    # inputs

# 	normalize = ""
# 	npypath = "npy_training/training_data_sce.npy"
   
	normalize = "_nbmean" 
	npypath = "npy_training/training_data_sce_nbymean.npy"

	outpath = "dictionary" #path for output
	dict_size = 80
	k_sparsity = 3
	lasso_sparsity = 0.2
	nmeasurements = 10
	...
	all_genes = "config/all_genes.npy" # all_genes 
	selected_genes = "config/selected_genes.txt" # selected genes
    
	L = np.load(all_genes)
	X = np.load(npypath)
	f = open(selected_genes)
	...
	# run SMAF
	U,V = smaf(X[:,train],dict_size,k_sparsity,lasso_sparsity,maxItr=10,use_chol=False,donorm=True,mode=1,mink=0.,doprint=True)
	nz = (U.sum(0) > 0)
	U = U[:,nz]
	print(U.shape)
	np.save('%s/%d_ms_%d_max%s/gene_modules.npy' % (outpath,nmeasurements,maxcomposition-1,normalize),U)
	if (mode == 'M') or (mode == 'G'):
		...
		for _ in range(2000):
			while True:
				if mode == 'M':
					phi = random_phi_subsets_m(nmeasurements,X.shape[0],(2,maxcomposition),d_thresh=0.8)
				elif mode == 'G':
					phi = random_phi_subsets_g(nmeasurements,X.shape[0],(1,maxcomposition),d_thresh=0.8)
		...
		f2 = open('%s/%d_ms_%d_max%s/simulation_results.txt' % (outpath,nmeasurements,maxcomposition-1,normalize),'w')
		...
		for i in xs:
			f1 = open('%s/%d_ms_%d_max%s/version_%d.txt' % (outpath,nmeasurements,maxcomposition-1,normalize,i),'w')
			phi = Phi[i]
